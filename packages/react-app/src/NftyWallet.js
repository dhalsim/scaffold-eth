...
export default function NftyWallet(props) {
  ...
  const [drawing, saveDrawingToLocalStorage] = useLocalStorage("drawing")
  const [viewDrawing, setViewDrawing] = useState()
  const [ipfsHash, setIpfsHash] = useState()
  const [ink, setInk] = useState({})
  const [renderKey, setRenderKey] = useState(Date.now())
  const [canvasKey, setCanvasKey] = useState(Date.now())
  const [bgImageIPFS, setBgImageIPFS] = useLocalStorage("bgImageIPFS")
  ...
  let accountWithCreateButton = (
    <div style={{ zIndex: 99, position: 'fixed', textAlign: 'right', right: 0, bottom: 20, padding: 10, backgroundColor: "#FFFFFF", borderRadius: 16 }}>
      <Row gutter={16} align={"middle"}>
        ...
        <Col>
          <Button style={{ marginRight: 8, marginTop: 8 }} shape="round" size="large" type="primary" onClick={() => {
            newInk()
        ...
        <TabPane tab={
          ...
          <div>
            <InkCanvas
              ...
              setInk={setInk}
              drawing={drawing}
              saveDrawingToLocalStorage={saveDrawingToLocalStorage}
              viewDrawing={viewDrawing}
              setViewDrawing={setViewDrawing}
              bgImageIPFS={bgImageIPFS}
              setBgImageIPFS={setBgImageIPFS}
              ipfsConfig={ipfsConfig}
              ipfsConfigInfura={ipfsConfigInfura}
              gasPrice={props.gasPrice}
              calculatedVmin={calculatedVmin}
